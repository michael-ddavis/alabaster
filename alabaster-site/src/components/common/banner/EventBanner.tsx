...
export default function EventBanner({
  ...
  return (
    <div ref={ref} className="sticky top-0 z-50 w-full bg-royal-blue/90 text-white/90 ring-1 ring-royal-blue/30 backdrop-blur-lg py-2">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8 py-2 flex-wrap sm:flex-nowrap">
        <span className="rounded-full bg-gold px-2.5 py-0.5 text-[11px] font-bold text-ink">UPCOMING</span>
        <div className="min-w-0 flex-1 truncate text-sm sm:text-base">
          <span className="font-serif">{title}</span>
          <span className="mx-2 text-white/70">• {new Date(date).toLocaleDateString()} {time}</span>
          {!left.over && (
            <span className="rounded-md border border-gold/40 bg-gold/10 px-2 font-bold text-sm whitespace-nowrap">
              {left.days}d {left.hours}h {left.minutes}m {left.seconds}s
            </span>
          )}
        </div>
        <Link to={`/gatherings#${id}`} className="rounded-md bg-seafoam px-3 py-1.5 text-sm font-semibold text-ink hover:brightness-110">See details</Link>
        <button
